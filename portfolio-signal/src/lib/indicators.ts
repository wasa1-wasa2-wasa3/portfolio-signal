...
export function calcSignals(prices: number[]): SignalResult {
  ...
  return {
    maS, rsiS, macdS, bbS, verdict, matchCount,
    rsiVal: rsi.toFixed(1),
    maVal: (close / ma25 * 100).toFixed(1),
    macdVal: macdVal.toFixed(2),
    bbVal: (std / sma20 * 100).toFixed(1),
    price: close,
    change: (close - prices[n - 2]) / prices[n - 2] * 100,
    prices,
  }
}
